...
class SimulationRobobo(IRobobo):
    ...
    def red_food_position(self) -> Position:
        """Get the position of the red foo to deliver at the base.

        This only works in the simulation.
        Trivially doesn't work when the simulation does not have a base.
        """
        if self._food is None:
            raise AttributeError("Scene does not have a red food")
    ...
    def _initialise_handles(self) -> None:
        ...
        try:
            ...
            self._food_script = self._get_childscript(self._food)
        except AttributeError:
